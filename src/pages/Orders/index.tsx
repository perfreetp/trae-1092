...
export const Orders: React.FC = () => {
  ...
  const handleGenerateOrder = () => {
    ...
    const newOrder = calculateParkingFee(billingPlate.trim(), billingEntryTime);
    addOrder(newOrder);
          ...
          <div className="w-96 rounded-xl bg-slate-800 p-6 animate-fade-in">
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                />
                {invoiceErrors.title && (
                  <p className="mt-1 text-xs text-red-400">{invoiceErrors.title}</p>
                )}
              </div>
              <div>
                ...
                />
                {invoiceErrors.taxNumber && (
                  <p className="mt-1 text-xs text-red-400">{invoiceErrors.taxNumber}</p>
                )}
              </div>
              <div>
                <label className="text-sm text-slate-300">
                ...
                />
                {invoiceErrors.email && (
                  <p className="mt-1 text-xs text-red-400">{invoiceErrors.email}</p>
                )}
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="text"
                    value={billingPlate}
                    onChange={(e) => setBillingPlate(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBillingEntryTime(new Date(e.target.value).toISOString())}
                    className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-white focus:border-blue-500 focus:outline-none"
                  />
                </div>
              </div>
                ...
                <div className="rounded-lg bg-slate-900 p-4 space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-slate-300">
                      停车时长：{formatDuration(billingResult.durationMinutes)}
                    </span>
                  </div>
